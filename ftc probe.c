...
void set_voltage(uint8_t motor , float duty) {
 ...
 case 0:
	 ...
	 else {   TIM4->CCR1 = ((int32_t)(TIM4->ARR + (TIM4->ARR * duty)));
  ...
  case 1:
	  ...
	  else {   TIM4->CCR2 = ((int32_t)(TIM4->ARR + (TIM4->ARR * duty)));
  ...
  case 2:
	  ...
	  else {   TIM4->CCR3 = ((int32_t)(TIM4->ARR + (TIM4->ARR * duty)));
  ...
  case 3:
	  ...
	  else {   TIM4->CCR4 = ((int32_t)(TIM4->ARR + (TIM4->ARR * duty)));
	  	  HAL_GPIO_WritePin(GPIOE, GPIO_PIN_14, GPIO_PIN_SET);  }
  break;
 }
}
void PID_controler(uint8_t motor , float result_speed,float speed){

 	float Kp = 17.0;
 	float Ki = 0.0;
 	float Kd = 1.0;

 	now_error = speed - result_speed ;

 	P  = Kp * now_error;
 	I  = I_old + Ki * now_error;
 	delta_error = now_error - old_error;
 	D  = Kd * delta_error;
 	u  = P  + I  + D ;
 	old_error = now_error;
 	I_old = I;
...
int main(void)
{
  ...
  {
    /* USER CODE END WHILE */
	  PID_controler(0 , result_speed_0 , 0.25);
